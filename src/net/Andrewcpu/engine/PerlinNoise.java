...
public class PerlinNoise {
     ...
     */
    private static float weigh(float x) {
     ...
     */
    private static float lerp(float weight, float a, float b) {
     ...
     * a * c + b * d
     */
    private static float dot(vec2 v0, vec2 v1) {
     ...
     */
    private static vec2 g(int x, int y) {
        if (x < 0) x = 0;
        if (y < 0) y = 0;
        if (x >= width) x = width;
